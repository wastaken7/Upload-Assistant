...
(function() {
    ...
    async function copyToClipboard(text, button, originalText) {
        try {
            ...
            setTimeout(() => {
                ...
                button.childNodes[button.childNodes.length - 1].nodeValue = originalText;
    ...
    function convertNodeToBBCode(node) {
        ...
        switch (tag) {
            case 'H1': return `[h1]${innerContent}[/h1]`;
            case 'H2': return `[h2]${innerContent}[/h2]`;
            case 'H3': return `[h3]${innerContent}[/h3]`;
            case 'H4': return `[h4]${innerContent}[/h4]`;
            case 'H5': return `[h5]${innerContent}[/h5]`;
            case 'H6': return `[h6]${innerContent}[/h6]`;
            case 'B':
            case 'STRONG':
            ...
            case 'U': return `[u]${innerContent}[/u]`;
            case 'S':
            case 'DEL':
            case 'STRIKE':
                return `[s]${innerContent}[/s]`;
            case 'SUB': return `[sub]${innerContent}[/sub]`;
            case 'SUP': return `[sup]${innerContent}[/sup]`;
            case 'SMALL': return `[small]${innerContent}[/small]`;
            case 'SPAN': {
                const style = node.getAttribute('style') || '';
                let open = '';
                let close = '';

                const sizeMatch = style.match(/font-size:\s*clamp\(\d+px,\s*(\d+)px,\s*\d+px\)/i) || style.match(/font-size:\s*(\d+)(?:px|pt)?/i);
                if (sizeMatch) {
            ...
            case 'P': {
                ...
                if (node.classList.contains('bbcode-rendered__center') || node.style.textAlign === 'center') {
                    open = '[center]';
                    close = '[/center]';
                } else if (node.classList.contains('bbcode-rendered__left') || node.style.textAlign === 'left') {
                    open = '[left]';
                    close = '[/left]';
                } else if (node.classList.contains('bbcode-rendered__right') || node.style.textAlign === 'right') {
                    open = '[right]';
                    close = '[/right]';
            ...
            case 'A': {
                ...
                if (href === innerContent || href === decodeURIComponent(innerContent)) {
                    return `[url]${href}[/url]`;
                }
                return `[url=${href}]${innerContent}[/url]`;
            }
            case 'IMG': {
                if (node.classList.contains('joypixels') || node.src.includes('joypixels')) {
            ...
            case 'TR': return `[tr]\n${innerContent}[/tr]\n`;
            case 'TH': return `[th]${innerContent}[/th]`;
            case 'TD': return `[td]${innerContent}[/td]`;
            case 'DETAILS': {
            ...
            case 'IFRAME': {
                const src = node.getAttribute('src') || '';
                const ytMatch = src.match(/youtube-nocookie\.com\/embed\/([a-zA-Z0-9_-]{11})/i) || src.match(/youtube\.com\/embed\/([a-zA-Z0-9_-]{11})/i);
                if (ytMatch) {
    ...
    function convertNodeToMarkdown(node) {
        ...
        switch (tag) {
            ...
            case 'TABLE': {
                ...
                for (let row of rows) {
                    const cells = Array.from(row.childNodes).filter(c => c.tagName === 'TD' || c.tagName === 'TH');
            ...
            case 'DETAILS': {
                ...
                const bodyEl = node.querySelector('div');
                const bodyContent = bodyEl ? convertNodeToMarkdown(bodyEl) : innerContent;
    ...
    function init() {
        ...
        if (!document.querySelector('meta[name="_base_url"]') && !document.querySelector('meta[name="csrf-token"]')) {
            return;
        }

        // Confirm this is a wiki, pages, rules, or articles page (where editing is generally restricted)
        const isTargetPage = window.location.pathname.match(/\/(wikis|wiki|pages|articles|rules)(\/|$)/i);
        if (!isTargetPage) {
            return;
        }

        const renderedBlocks = document.querySelectorAll('.bbcode-rendered:not([data-copier-processed])');
        renderedBlocks.forEach(block => {
            ...
            bbBtn.className = 'form__button form__button--outlined form__button--centered';
            ...
            bbBtn.innerHTML = '<i class="fa fa-code" style="margin-right: 4px;"></i> Copy BBCode';
            ...
            mdBtn.className = 'form__button form__button--outlined form__button--centered';
            ...
            mdBtn.innerHTML = '<i class="fa fa-copy" style="margin-right: 4px;"></i> Copy Markdown';
