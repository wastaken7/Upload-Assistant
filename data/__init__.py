...
if not CONFIG_PATH.exists():
    if LEGACY_CONFIG_PATH.is_file():
        shutil.move(str(LEGACY_CONFIG_PATH), CONFIG_PATH)
        warnings.warn(f"Moved legacy configuration to {CONFIG_PATH}. The checkout is now free of runtime configuration.", stacklevel=2)
    else:
