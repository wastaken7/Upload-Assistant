...
async def process_media_files(prep_instance: Any, meta: Meta, videoloc: str, bdinfo: dict[str, Any]) -> tuple[str, str, str, str, str, Optional[dict[str, Any]], str]:
    ...
    if meta.is_disc == "BDMV":
        ...
        if meta.category == "BOOK" or str(meta.manual_category or "").upper() == "BOOK":
...
async def search_metadata(
    ...
    if meta.imdb_info is None and imdb_id_value != 0 and meta.category not in ("BOOK", "GAME"):
        imdb_info = await imdb_manager.get_imdb_info_api(imdb_id_value, manual_language=meta.manual_language, debug=meta.debug)
...
async def finalize_metadata(
    ...
    if meta.category in ("TV", "MOVIE"):
        ...
        if meta.tag == "-SubsPlease":  # SubsPlease-specific
            ...
            meta.episode_title = ""
            if (
                (bitrate.isdigit() and int(bitrate) >= 8000000) or (bitrate_oldMediaInfo.isdigit() and int(bitrate_oldMediaInfo) >= 8000000) and meta.resolution == "1080p"
            ):  # 8Mbps for 1080p
                meta.service = "CR"
            elif (
                ...
                (bitrate.isdigit() and int(bitrate) >= 4000000) or (bitrate_oldMediaInfo.isdigit() and int(bitrate_oldMediaInfo) >= 4000000) and meta.resolution == "720p"
            ):  # 4Mbps for 720p
                meta.service = "CR"
