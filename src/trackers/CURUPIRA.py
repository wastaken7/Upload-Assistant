...
class CURUPIRA:
    ...
    def get_source(self, meta: Meta) -> Optional[str]:
        ...
        if "BLU" in source_upper:
            ...
            return "WEBRip" if meta.get('type') == 'WEBRIP' else "WEB-DL"
    ...
    async def _prepare_files(self, meta: Meta) -> Optional[dict[str, Any]]:
        ...
        files = {
            'nzb_file': (os.path.basename(nzb_path), nzb_content, 'application/x-nzb')
        }
    ...
    async def _prepare_data(self, meta: Meta, tracker_cfg: Config) -> dict[str, Any]:
        ...
        if meta["category"] in ("TV", "MOVIE") or meta.get("audiobook", False):
            data['mediainfo_text'] = await self.get_media_info(meta)

        # Quality (optional)
        quality = meta.get('resolution')
        ...
        # Anonymous (optional)
        anon = 0 if meta.get('anon') == 0 and not tracker_cfg.get('anon', False) else 1
        if anon:
            data['anonymous'] = 'true'
    ...
    async def upload(self, meta: Meta, _disctype: str) -> Optional[bool]:
        ...
        if meta.get('debug', False):
            console.print("[cyan]CURUPIRA Upload (DEBUG MODE):[/cyan]")
