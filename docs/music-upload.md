...
Discogs complements this flow for an exact, known pressing only: pass a release/master ID or URL and the program makes at most two cached, read-only API requests. It does not title-search Discogs, because that could choose the wrong edition. The exact release can provide concrete date, label, catalogue number, country, genres and medium; a master can corroborate the original group year. Add `DEFAULT["music_discogs_token"]` optionally to use your own Discogs rate limit. The token is never saved in the metadata snapshot or logged.
